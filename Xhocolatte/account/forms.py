...
class RegistrationForm(forms.ModelForm):
    ...
    user_name = forms.CharField(
        label='Usuario', min_length=4, max_length=50, help_text='Required')
    ...
    def clean_email(self):
        ...
        if UserBase.objects.filter(email=email).exists():
            raise forms.ValidationError(
                'Porfavor usa otra email, ese ya esta cogido.')
        return email
...
class UserEditForm(forms.ModelForm):
    email = forms.EmailField(
        label='Email (no se puede cambiar)', max_length=200, widget=forms.TextInput(
            attrs={'class': 'form-control mb-3', 'placeholder': 'Email', 'id': 'form-email', 'readonly': 'readonly'}))

    first_name = forms.CharField(
        label='Primer nombre', min_length=4, max_length=50, widget=forms.TextInput(
            attrs={'class': 'form-control mb-3', 'placeholder': 'Primer Nombre', 'id': 'form-firstname'}))
...
class PwdResetForm(PasswordResetForm):
    ...
    def clean_email(self):
        ...
        if not u:
            raise forms.ValidationError(
                'Desafortunadamente no hemos encontrado ese correo')
        return email
